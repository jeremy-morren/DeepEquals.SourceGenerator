...
// Cycle handling, fingerprint, hashing and bit-block benchmarks (results in docs/FutureNotes.md): each class names what
// it is meant to expose. They share the in-process configuration, so every target framework hosts its own run, and the
// downstream models, so the data is the data the smoke checks assert on.
...
namespace DeepEquals.Benchmarks
{
    ...
    internal static class Modes
    {
        public static IEqualityComparer<TreeNode> Tree(string mode) =>
            mode switch
            {
                "Path" => DownstreamPathContext.TreeNode,
                "Tree" => DownstreamTreeContext.TreeNode,
                _ => DownstreamContext.TreeNode
            };

        public static IEqualityComparer<ListNode> Chain(string mode) =>
            mode switch
            {
                "Path" => DownstreamPathContext.ListNode,
                "Tree" => DownstreamTreeContext.ListNode,
                _ => DownstreamContext.ListNode
            };

        public static IEqualityComparer<HashSet<TreeNode>> TreeSet(string mode) =>
            mode switch
            {
                "Path" => DownstreamPathContext.HashSetOfTreeNode,
                "Tree" => DownstreamTreeContext.HashSetOfTreeNode,
                _ => DownstreamContext.HashSetOfTreeNode
            };
    }
    ...
    public class RetainedPairsBenchmarks
    {
        ...
        private TreeNode Build()
        {
            ...
            {
                var node = new TreeNode { Value = 0, Children = [] };
                for (var i = 1; i < 64; i++)
                    node = new TreeNode { Value = i, Children = [node] };
            ...
            { 
                Value = -1, 
                Children = Enumerable.Range(0, leaves)
                    .Select(i => new TreeNode { Value = i, Children = [] })
                    .ToList() 
            };
        }
    ...
    public class FingerprintBenchmarks
    {
        ...
        public void Setup()
        {
            _comparer = Depth switch
            {
                1 => DownstreamMatch1WideContext.HashSetOfListNode,
                2 => DownstreamMatch2Context.HashSetOfListNode,
                _ => DownstreamContext.HashSetOfListNode
            };
            _a = Data.ChainSet(65);
            _b = Data.ChainSet(65);
        }
    ...
    public class HashTableBenchmarks
    {
        ...
        public void Setup()
        {
            ...
            _customerComparer = Comparer == "generated"
                ? DownstreamContext.Customer
                : new BuiltIn.Comparer<Customer>(BuiltIn.CustomerEquals, BuiltIn.CustomerHash);
            _orderComparer = Comparer == "generated" 
                ? DownstreamContext.Order
    ...
    public class BitBlockBenchmarks
    {
        ...
        public void Setup()
        {
            ...
            if (MismatchLast) 
                _pb[Length - 1] = _pb[Length - 1] with { X = -1 };
#endif
        }
