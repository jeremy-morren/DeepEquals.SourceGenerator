...
namespace DeepEquals.Downstream
{
    public static class Data
    {
        ...
        public static Guid[] Guids(int seed, int count)
        {
            ...
            for (var i = 0; i < count; i++) 
                values[i] = new Guid(seed, (short)i, (short)(i >> 16), 1, 2, 3, 4, 5, 6, 7, 8);
            return values;
        }
        ...
        public static Point3[] Point3s(int seed, int count)
        {
            ...
            for (var i = 0; i < count; i++) 
                values[i] = new Point3(r.NextDouble() + 1, r.NextDouble() + 1, r.NextDouble() + 1);
            return values;
        }
#endif

        private static long NextInt64(Random r) => ((long)r.Next() << 32) | (uint)r.Next();
    }
}
