...
namespace DeepEquals.Downstream
{
    ...
    public static class MicroBench
    {
        ...
        public static string ColdStart()
        {
            ...
            Cold(text, "Graph", () => 
                DownstreamContext.Order.Equals(Data.Order(2), Data.Order(2)) && 
                DownstreamContext.Order.GetHashCode(Data.Order(2)) != 0);
            Cold(text, "Path", () => 
                DownstreamPathContext.TreeNode.Equals(Data.Tree(6, 2, 4), Data.Tree(6, 2, 4)) && 
                DownstreamPathContext.TreeNode.GetHashCode(Data.Tree(6, 2, 4)) != 0);
            Cold(text, "Tree", () => 
                DownstreamTreeContext.TreeNode.Equals(Data.Tree(6, 2, 4), Data.Tree(6, 2, 4)) && 
                DownstreamTreeContext.TreeNode.GetHashCode(Data.Tree(6, 2, 4)) != 0);
            Cold(text, "bit blocks", () => 
                DownstreamContext.ArrayOfDouble.Equals(Data.Doubles(1, 64), Data.Doubles(1, 64)) && 
                DownstreamContext.ArrayOfDouble.GetHashCode(Data.Doubles(1, 64)) != 0);
        ...
        private static void Cold(StringBuilder text, string name, Func<bool> first)
        {
            ...
            var ok = first();
            watch.Stop();
            var elapsed = watch.ElapsedTicks * (1e6 / Stopwatch.Frequency);
            text.AppendLine(
                Pad(name, 32) + 
                $"{elapsed.ToString("N1", CultureInfo.InvariantCulture)} us" + 
                (ok ? string.Empty : " (unexpected result)"));
        }
        ...
        private static string Ratio(double generated, double builtIn) => 
            builtIn <= 0 ? "-" : (generated / builtIn).ToString("0.00", CultureInfo.InvariantCulture) + "x";
        ...
        /// <summary>
        /// A quiet period for background compilation. The browser's single thread cannot sleep, and does not need to.
        /// </summary>
