...
/// <summary>
/// The trimming warning boundary. A consumer that only touches comparers reachable without reflection
/// must publish clean; a consumer that touches one that needs a delegate must be warned, at its own
/// line rather than inside a generated file. The design's "toolchain warnings follow the unsafe path"
/// section is what these two assert.
/// </summary>
...
public sealed class TrimmingTests
{
    ...
    {
        // net10.0: both registered types take the UnsafeAccessor path, and this consumer touches only
        // the plain comparer.
        CommandResult publish = Dotnet.Publish(_output, SmokePaths.Consumer("Trimming.Safe"));
    ...
    public void A_consumer_on_the_unsafe_path_is_warned_at_its_own_line()
    {
        ...
        CommandResult publish = Dotnet.Publish(_output, SmokePaths.Consumer("Trimming.Unsafe"));

        publish.ExitCode.Should().Be(0, "warnings are not errors for this consumer");

        List<string> warnings = publish.TrimmingWarnings.ToList();
