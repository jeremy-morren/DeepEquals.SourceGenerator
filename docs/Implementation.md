...
| Member | Shape |
|---|---|
| header | filled from the embedded template `Templates/AutoGeneratedHeader.cs`: an `<auto-generated>` notice with the generator version; in the file of each type on the delegate accessor path, a trimming notice saying why and which attributes its comparer carries; in the context file only, a block comment with the context name, its registered roots, custom comparers, the options in effect, the language version and probed capabilities, the diagnostics reported for the context, and documentation links; then `#nullable enable annotations` when C# 8 or later; then one `#pragma warning disable CSnnnn // Suppress …` line for each warning the generator's own code can raise (CS0108, CS0162, CS0168, CS0219, CS1591, CS8019, CS8632, and CS9192 and CS9193 where `Nullable.GetValueRefOrDefaultRef` is called); then, after a blank line, one line for each warning the compared types' own APIs raise, which the consuming project already accepted by using them: CS0612 and CS0618 for obsolete members, and every custom `[Obsolete]` diagnostic id, `[Experimental]` id and preview-feature id, each naming the symbol that carries it. Everything comes from the model, so the header is deterministic |
