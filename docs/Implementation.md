...
Hashing never looks at object identity. Null hashes to 0. Every library-defined collection hash returns 0 for null and a nonzero value for empty: if the count is 0 and the raw hash came out as 0, the result is 1. `Combine` and the streaming forms are xxHash32 over 32-bit words, or under `Hashing = XxHash64` xxHash64 over 64-bit words, each with its own per-process random seed ([§8](#8-hash-runtime)). Under `Tree` a hash core that reaches a cycle takes the depth and carries the same guard as the comparison.

Equality and hashing of a built-in leaf operate on its storage bits, never on a numeric conversion: float, double and `Half` go through `DeepEqualsHelpers.FloatBits`, `DoubleBits` and `HalfBits`, and decimal and `Guid` through their 64-bit or 32-bit storage words. The only casts in a hash word are integer reinterpretations and extensions.

**Width.** Under `XxHash32` every word is an `int`, and a leaf wider than 32 bits contributes each of its 32-bit words. Under `XxHash64` every hash core returns a `ulong`: a 64-bit leaf is one word, a 128-bit leaf two, a nested hash one word carrying all its bits, and a 32-bit hash is narrow. The narrow words of one stream are paired in order with `Pack(a, b) = ((ulong)(uint)a << 32) | (uint)b`, an odd one sits alone zero-extended, and the wide words follow. The public `GetHashCode` folds the word with `DeepEqualsHashCode64.ToInt32`: 0 stays 0, and every other word folds to a nonzero value, since `Fold(0x0000000100000001)` would otherwise be 0 and an empty collection would hash like null. The matching fingerprints inside unordered comparison stay 32-bit, folded.
...
Every file starts with the same header, computed once per context; the context file lists the whole closure where a type file lists the one type it holds. A type file's header and skeleton are written only after its body turns out non-empty, so a type with nothing to emit costs neither. The members:

| Member | Shape |
|---|---|
| header | filled from the embedded template `Templates/AutoGeneratedHeader.cs`: an `<auto-generated>` block with the generator version, the context name, its registered roots, every public comparer with its kind (a type file: the one type of the file), custom comparers, types on the delegate accessor path, the options in effect, the language version and probed capabilities, the diagnostics reported for the context, and documentation links; then `#pragma warning disable CS0612, CS0618, CS8632` plus every custom obsolete, `[Experimental]` and preview-feature id carried by a referenced symbol; then `#nullable enable annotations` when C# 8 or later. Everything comes from the model, so the header is deterministic |
...
| cores | `private static` methods on the context: `Equals_{T}`, `EqualsExact_{T}`, `EqualsMembers_{T}`, `EqualsBoxed_{S}`, `Equals_SpanOf{T}`, `GetHashCode_{T}`, `ShallowHashCode_{T}`, `HashMembers_{T}`, `ShallowHashMembers_{T}`, `MatchHashCode_{T}_L{n}`, `MatchHashMembers_{T}_L{n}`, `GetHashCode_SpanOf{T}`. Hash cores return `int`, or `ulong` under `XxHash64` |
...
| ops structs | `private struct {T}Ops` implementing `IDeepEqualsElementOps<T>`, `IDeepEqualsStatelessElementOps<T>`, or under `Tree` `IDeepEqualsDepthElementOps<T>`; `{T}ShallowOps` where a same-component collection hash needs level 0; hash-only ops implement `IDeepEqualsHashOps<T>` or `IDeepEqualsHashOps64<T>`, or their depth forms under `Tree` |
...
| `long`, `ulong`, `nint`, `nuint` | `==` | the two 32-bit words, or one 64-bit word |
| `Int128`, `UInt128` | `==` | `Hash(v)` over its words |
| `BigInteger`, `Rune` | `==` | `GetHashCode()` |
| `float`, `double`, `Half` | the bits from `DeepEqualsHelpers.FloatBits`, `DoubleBits` or `HalfBits` must match; each is `BitConverter`'s intrinsic where the asset has one, otherwise `Unsafe.As` over a reference | the same bits (two 32-bit words or one 64-bit word for double) |
| `decimal` | bitwise: `DeepEqualsHelpers.DecimalEquals`, two 64-bit compares over the storage, which is the four `GetBits` words in storage order | the four 32-bit words, or the two 64-bit words `DecimalLo64`/`DecimalHi64` |
...
| `Guid` | `==` | the four 32-bit words (`DeepEqualsHelpers.GuidWord`) or two 64-bit words (`GuidLo64`/`GuidHi64`) |
...
`DeepEqualsHashCode64` is the same for xxHash64 over `ulong` words, with its own seed: `Combine` over 1 to 32 words from the same generator script at `-Width 64`, `Hash` for the 64-bit and 128-bit leaves and strings, `HashSpan` and `Streaming`, and the boundary helpers `Pack`, `Narrow`, `Fold`, `FoldNonZero` and `ToInt32` ([§3](#3-the-hash-function)). Both classes have `HashSpan(span, depth)` overloads over the depth ops for `Tree`.

`DeepEqualsBlocks` holds the bit-block paths: `BlockEquals` over two spans' bytes, `HashBytes32`/`HashBytes64` as seeded XxHash3 from `System.IO.Hashing` with the 64-bit process seed and never 0, `HashBlock` over a span, `HashReadOnlyList`, `HashList` and `HashEnumerable` with their span shortcuts and pooled copies, and `HasSize<T>`. It is absent from the netstandard2.1 asset.
