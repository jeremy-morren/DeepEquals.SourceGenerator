...
From C# 10 the context's namespace is file-scoped (`namespace X;`), one indentation level less on every line; below C# 10 it is a block. The framework classes generated code names most often are named through aliases: `DeqHelpers` (`DeepEqualsHelpers`), `DeqHashCode`, `DeqState`, `DeqCollections`, `DeqUnordered`, `DeqBlocks` and `DeqReflection`, so a call reads `DeqHelpers.FloatBits(x.A)`. Each file declares only the aliases its body uses, inside its namespace declaration (at the top of the file for a context in the global namespace): there an alias shadows a type of the same name in an enclosing namespace and a `global using` alias of the same name, and clashes only with a type of that name declared in the context's own namespace. Every other name is written `global::`-qualified.
...
The context file starts with the full `<auto-generated>` notice, naming the generator's version, and a type file with the one-line `// <auto-generated/>` marker, which tools read the same way; then each has the compiler directives. Only the context file carries the block comment describing the context, so a type file's header is the same few lines whatever the closure's size or the number of roots. A type file's header and skeleton are written only after its body turns out non-empty, so a type with nothing to emit costs neither. The members:

| Member | Shape |
|---|---|
| header | filled from the embedded template `Templates/AutoGeneratedHeader.cs`: in the context file an `<auto-generated>` notice with the generator version, in a type file the one-line `// <auto-generated/>` (each comparer also carries the version in `[GeneratedCode]`); in the file of each type on the delegate accessor path, a trimming notice saying why and which attributes its comparer carries; in the context file only, a block comment with the context name, its registered roots, custom comparers, the options in effect, the language version and probed capabilities, the diagnostics reported for the context, and documentation links; then `#nullable enable` when C# 8 or later, full analysis included; then one `#pragma warning disable ID // Suppress …` line for each warning that naming the compared types' own APIs raises and nothing else can avoid, each naming its symbol: the warning id of an `[Obsolete]` type in a name generated code writes (CS0612, CS0618 or its custom `DiagnosticId`), of an `[Obsolete]` member read directly or through its getter because no accessor exists, and every `[Experimental]` and preview-feature id. A file whose types use none of these has no pragma at all. Everything comes from the model, so the header is deterministic |
| warnings | generated code raises none of its own: every public member has XML documentation, a lower-case type name is redeclared verbatim (`@name`, CS8981), `Nullable.GetValueRefOrDefaultRef` is called with `in` on a variable only. Each file has full `#nullable enable`; the private implementation, after the public surface, sits under `#nullable disable annotations`, because one core serves every nullability spelling of a type (`List<string>` and `List<string?>` are one runtime type) and C# checks nested nullability at every call (CS8620); an oblivious parameter accepts every spelling, and the bodies keep full flow analysis. The generator tests compile every generated file at the highest warning level with documentation diagnostics and fail on any warning |
| bodies | a method whose body is a single `return` is written expression-bodied, the arrow on the signature's line: `public bool Equals(Person? x, Person? y) => Equals_Person(x, y);`. A multi-line expression, such as a member chain, starts on the line after the arrow. Whether a body is only a return often depends on guards and preludes that write nothing for most types, so `CodeWriter.Member` decides when the body closes, from what was written |
...
Without state the method is expression-bodied: `public bool Equals(Node? x, Node? y) => Equals_Node(x, y);`. Under `Path` the entry point is the same and each guarded core rolls its pair back on return; under `Tree` it is:
...
A struct or `decimal` payload of a nullable that is a variable (a field read directly or through an accessor, or a parameter) is reached by reference instead, through the runtime's `Nullable.GetValueRefOrDefaultRef` where it exists (.NET 7 and later): `Equals_Point(global::System.Nullable.GetValueRefOrDefaultRef(in x.Opt), …)`. With the nullable itself read in place ([§6.1](#61-class)), the payload is compared where it lives. A nullable that is a value, such as a getter's result, is read with `GetValueOrDefault()`: the method takes `ref readonly` from .NET 8 and would warn on it (CS9193), and would have to copy it anyway.
...
**Obsolete members and types.** A member marked `[Obsolete]`, on the field, the property or its getter, is not named where `[UnsafeAccessor]` can read its storage instead, so it raises no warning. Without an accessor, one at warning level is read like any other under a header pragma for its warning id, and one at error level through a delegate, since CS0619 cannot be suppressed. Naming a type uses every obsolete type in the name (the type, its containing types, type arguments and element types); the comparer class and convenience property of such a type repeat its `[Obsolete]` with the message, error flag, `DiagnosticId` and `UrlFormat`, so a caller sees what using the type itself would show, and the comparer's members, an obsolete context, name the type without a report. The private cores that name it cannot be marked the same way: their callers include the comparers of non-obsolete types holding it, which would then report the use, so the header suppresses the type's warning id. A type obsolete as an error can be compared only when the context class is itself `[Obsolete]`, which makes every generated member an obsolete context; otherwise [DEQ038](Diagnostics.md#registrations) stops the context.
