...
public static class DeepEqualsBlocks
{
    ...
    private static long s_seed = unchecked((long)DeepEqualsHashCode.RandomSeed());
    ...
    /// <summary>
    /// The hash of a read-only list of bit-block elements: its span when it has one, otherwise a copy through its
    /// indexer, the only way in; a short one through the stack, with no pool traffic for the common small case.
    /// </summary>
    public static int HashReadOnlyList<T>(IReadOnlyList<T> list) where T : unmanaged
    {
        if (DeepEqualsCollections.TryGetSpan(list, out var span))
            return HashBlock(span);

        var count = list.Count;
        if (count <= StackCopyBytes / Unsafe.SizeOf<T>())
        {
    ...
    /// <summary>
    /// The hash of a list of bit-block elements: its span when it has one, otherwise a copy through <c>CopyTo</c>,
    /// one bulk move into a pooled buffer, which beats an indexer walk even for a short list.
    /// </summary>
    ...
    /// copied into a pooled buffer that grows as needed. An advertised count only sizes the first buffer;
    ...
    {
        if (DeepEqualsCollections.TryGetSpan(items, out var span))
            return HashBlock(span);
