...
public static class DeepEqualsUnordered
{
    ...
    public static bool SetEquals<T, TOps>(IEnumerable<T> x, IEnumerable<T> y, int count, int maxCollisionRun, ref DeepEqualsState state)
        ...
        => EqualsCore<T, StatefulOps<T, TOps>, SetFill<T, StatefulOps<T, TOps>>>(x, y, count, maxCollisionRun, ref state, 0);
        ...
        where TOps : struct, IDeepEqualsStatelessElementOps<T>
    {
        ...
        return EqualsCore<T, StatelessOps<T, TOps>, SetFill<T, StatelessOps<T, TOps>>>(x, y, count, maxCollisionRun, ref none, 0);
    }
    ...
    public static bool DictionaryEquals<TKey, TValue, TOps>(IEnumerable<KeyValuePair<TKey, TValue>> x, IEnumerable<KeyValuePair<TKey, TValue>> y, int count, int maxCollisionRun, ref DeepEqualsState state)
        ...
        => EqualsCore<KeyValuePair<TKey, TValue>, StatefulOps<KeyValuePair<TKey, TValue>, TOps>, DictionaryFill<TKey, TValue, StatefulOps<KeyValuePair<TKey, TValue>, TOps>>>(x, y, count, maxCollisionRun, ref state, 0);
    ...
    {
        DeepEqualsState none = default;
        return EqualsCore<KeyValuePair<TKey, TValue>, StatelessOps<KeyValuePair<TKey, TValue>, TOps>, DictionaryFill<TKey, TValue, StatelessOps<KeyValuePair<TKey, TValue>, TOps>>>(x, y, count, maxCollisionRun, ref none, 0);
    }
    ...
    {
        DeepEqualsState none = default;
        return EqualsCore<T, DepthOps<T, TOps>, SetFill<T, DepthOps<T, TOps>>>(x, y, count, maxCollisionRun, ref none, depth);
    }
    ...
    {
        DeepEqualsState none = default;
        return EqualsCore<KeyValuePair<TKey, TValue>, DepthOps<KeyValuePair<TKey, TValue>, TOps>, DictionaryFill<TKey, TValue, DepthOps<KeyValuePair<TKey, TValue>, TOps>>>(x, y, count, maxCollisionRun, ref none, depth);
    }
    ...
    private static bool EqualsCore<T, TW, TFill>(IEnumerable<T> x, IEnumerable<T> y, int count, int maxCollisionRun, ref DeepEqualsState state, int depth)
        where TW : struct, IUnorderedOps<T>
        where TFill : struct, IFill<T>
    {
        if (x is null)
        ...
        try
        {
            ...
            var xSum = default(TFill).Fill(x, xs, xk, count, depth);
            var ySum = default(TFill).Fill(y, ys, yk, count, depth);
    ...
    /// <summary>
    /// Materializes one side of a comparison: entries, packed <c>(hash, index)</c> keys, and the unchecked hash sum. A
    /// set or dictionary walks through its own struct enumerator, so no interface call per element.
    /// </summary>
    private interface IFill<T>
    {
        int Fill(IEnumerable<T> source, T[] entries, long[] keys, int count, int depth);
    }

    private readonly struct SetFill<T, TW> : IFill<T>
        where TW : struct, IUnorderedOps<T>
    {
        public int Fill(IEnumerable<T> source, T[] entries, long[] keys, int count, int depth)
        {
            var n = 0;
            var sum = 0;
            if (source is HashSet<T> set)
            {
                foreach (var element in set)
                    sum = unchecked(sum + Store<T, TW>(source, entries, keys, count, depth, element, n++));
            }
            else
            {
    ...
    private readonly struct DictionaryFill<TKey, TValue, TW> : IFill<KeyValuePair<TKey, TValue>>
        where TKey : notnull
        where TW : struct, IUnorderedOps<KeyValuePair<TKey, TValue>>
    {
        public int Fill(IEnumerable<KeyValuePair<TKey, TValue>> source, KeyValuePair<TKey, TValue>[] entries, long[] keys, int count, int depth)
        {
            var n = 0;
            var sum = 0;
            if (source is Dictionary<TKey, TValue> dictionary)
            {
                foreach (var entry in dictionary)
                    sum = unchecked(sum + Store<KeyValuePair<TKey, TValue>, TW>(source, entries, keys, count, depth, entry, n++));
            }
            else
            {
    ...
    /// <summary>
    /// Stores the <paramref name="n"/>th entry of a side, with its packed key, and returns its fingerprint for the hash
    /// sum. The counters stay with the caller, by value: a loop counter passed by reference cannot live in a register.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int Store<T, TW>(IEnumerable<T> source, T[] entries, long[] keys, int count, int depth, T element, int n)
        where TW : struct, IUnorderedOps<T>
    {
