...
public static class DeepEqualsHelpers
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int DecimalWord(in decimal value, int index) => Word(ref Unsafe.AsRef(in value), index);

    /// <summary>One of the four 32-bit words of a <see cref="Guid"/>; <see cref="Guid.GetHashCode"/> ignores six of its bytes.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static int GuidWord(in Guid value, int index) => Word(ref Unsafe.AsRef(in value), index);

    /// <summary>The 32-bit word at <paramref name="index"/> of a value's storage, read in place.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static int Word<T>(ref T value, int index) where T : unmanaged => Unsafe.Add(ref Unsafe.As<T, int>(ref value), index);
