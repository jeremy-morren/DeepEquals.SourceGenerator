...
internal sealed class Emitter
{
    ...
    /// <summary>
    /// Whether a wide leaf reached by reference contributes its words in place.
    /// Only on the .NET 8 asset, the one with <c>[UnsafeAccessor]</c> but not its generic form:
    /// its JIT copies a record struct's decimal with one vector store from which the four-byte word reads do not forward,
    /// and reading in place halves the struct's hash (FutureNotes 2.7);
    /// on .NET 10 the copy is register-promoted and reading in place costs about a nanosecond instead.
    /// </summary>
    private bool WordsInPlace => _model.Capabilities is { HasUnsafeAccessor: true, HasGenericUnsafeAccessor: false };

    /// <summary>
    /// Reads a wide leaf into <paramref name="local"/> and appends its 32-bit words: always the storage bits and never a numeric conversion.
    /// A 16-byte leaf or a <c>DateTimeOffset</c> reached by <paramref name="reference"/> (a field, or an accessor's <c>ref</c>)
    /// is read word by word in place instead, so no copy stands between the storage and the stream. False when the leaf is not wide.
    /// </summary>
    ...
    /// <c>MaxSwitchCases</c> one lookup in a prebuilt map (<see cref="UseSwitch"/>), with the cases most likely behind
    /// the static type tested ahead of the map (<see cref="Chained"/>): the leaves that dominate real payloads, then the
    /// closure's own types, up to <c>MaxSwitchCases</c> of them. A test costs 0.3 ns on .NET 10 and 1.5 ns on .NET 8
    /// against a map lookup of 4 and 22 (FutureNotes 2.7), so the chain wins for a dozen cases, and a user type behind
    ...
    private DispatchCases DispatchPlan(TypeModel type)
    {
        ...
        return new DispatchCases
        {
            ...
            UseSwitch = useSwitch,
            Chained = useSwitch 
                ? HotLeafCases(exact).Concat(OwnCases(exact).Take(_model.Options.MaxSwitchCases)).ToList() 
                : [],
        };
    }
    ...
    private IEnumerable<DispatchCase> OwnCases(List<DispatchCase> exact) =>
        exact.Where(c => 
                c.ResolvedAssignable < 0 && 
                Type(c.TypeId) is var t &&
    ...
    /// <summary>
    /// Whether a dispatch map is keyed by type handle rather than Type: on .NET 8 and later,
    /// where the lookup measured 4 to 12 ns faster (FutureNotes 2.7).
    /// On .NET Framework <c>IntPtr</c> has no <c>IEquatable</c>, so the default comparer boxes,
    /// and <c>TypeHandle.Value</c> is a 3.6 ns call: the handle map measured 12.6 ns against 9.6 by Type.
    /// </summary>
    ...
    private void EntryWords(TypeModel part, string value, bool forMatching, int level, bool sameScc, string local, List<string> words)
    {
        ...
        words.Add(forMatching 
            ? FingerprintWord(part, value) 
            : HashOrOmit(part, value, level, sameScc) ?? "0");
    }
    ...
    private void EmitAccessors(TypeModel type)
    {
        ...
        // so one holder per definition carries every accessor of that definition, whichever types read them
        // Call sites name the constructed arguments
        foreach (var member in type.Members)
        {
            if (member.Access != MemberAccess.UnsafeAccessor || 
                !member.GenericAccessor || 
                !_emittedAccessors.Add(member.GenericHolderName))
                continue;
            ...
            using (_w.Block($"private static class {member.GenericHolderName}{member.GenericTypeParameters}{member.GenericConstraints}"))
            {
                ...
                {
                    var openReceiver = accessor.DeclaringTypeIsValueType 
                        ? $"ref {accessor.OpenDeclaringTypeGlobalName} o" 
                        : $"{accessor.OpenDeclaringTypeGlobalName} o";
    ...
    /// <summary>
    /// Every generic accessor holder with its accessors in name order, over the whole closure; built on first use.
    /// </summary>
