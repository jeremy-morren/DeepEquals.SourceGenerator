...
public sealed class StateTests
{
    ...
    {
        object a = new(), b = new();
        var state = new DeepEqualsState(100);
        try
        {
    ...
    public void Spills_at_the_ninth_pair_and_keeps_every_earlier_pair()
    {
        ...
        pools.Pairs.Returned.Should().OnlyContain(array => 
            Array.TrueForAll(array, p => p.X == null && p.Y == null), "reference-bearing arrays are cleared on return");
    }
    ...
    public void Grows_past_the_initial_spill_capacity_and_still_finds_every_pair()
    {
        ...
        var state = new DeepEqualsState(100_000);
        try
        {
            for (var i = 0; i < n; i++) state.TryEnter(i % 3, xs[i], ys[i]).Should().BeTrue();
    ...
    public void Rollback_forgets_only_pairs_after_the_mark_inline_and_spilled()
    {
        ...
        try
        {
            for (var i = 0; i < 5; i++) state.TryEnter(1, xs[i], ys[i]);
            var inlineMark = state.Mark();
            for (var i = 5; i < 8; i++) state.TryEnter(1, xs[i], ys[i]);
            state.Rollback(inlineMark);
            state.Count.Should().Be(5);
            for (var i = 5; i < 8; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeTrue("rolled-back inline pairs are novel again");
            for (var i = 0; i < 5; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeFalse();

            // A mark taken inline, a trial that spills and grows, then a rollback to the inline mark.
            state.Rollback(inlineMark);
            for (var i = 5; i < 64; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeTrue();
            state.HasSpilled.Should().BeTrue();
            state.Rollback(inlineMark);
            state.Count.Should().Be(5);
            state.HasSpilled.Should().BeTrue("rollback does not un-spill");
            for (var i = 0; i < 5; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeFalse("pairs before the mark survive");
            for (var i = 5; i < 64; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeTrue("pairs after the mark were forgotten");

            // Rollback across a growth boundary in spilled mode.
            var spilledMark = state.Mark();
            var more = Objects(300);
            for (var i = 0; i < 300; i++) state.TryEnter(2, more[i], more[i]).Should().BeTrue();
            state.Rollback(spilledMark);
            state.Count.Should().Be(spilledMark);
            for (var i = 0; i < 64; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeFalse("the table before the trial is intact after growth and rollback");
            for (var i = 0; i < 300; i++) state.TryEnter(2, more[i], more[i]).Should().BeTrue();
        }
        finally
        {
    ...
    public void Budget_applies_only_to_novel_insertions()
    {
        ...
        var state = new DeepEqualsState(10);
        try
        {
            for (var i = 0; i < 10; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeTrue();
    ...
    public void Budget_smaller_than_the_inline_buffer_is_honoured()
    {
        ...
        var state = new DeepEqualsState(1);
    ...
    public void Failed_growth_leaves_the_old_state_intact_and_disposable()
    {
        ...
        try
        {
            for (var i = 0; i < 32; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeTrue();   // exactly the initial spill capacity
            ...
            for (var i = 0; i < 32; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeFalse("the old table survived the failed growth");
    ...
    public void Failed_index_rent_during_spill_returns_the_pair_array()
    {
        ...
        try
        {
            for (var i = 0; i < 8; i++) state.TryEnter(1, xs[i], ys[i]);
            ...
            state.TryEnter(1, xs[8], ys[8]).Should().BeTrue();
            for (var i = 0; i < 8; i++) state.TryEnter(1, xs[i], ys[i]).Should().BeFalse();
        }
        finally
        {
            state.Dispose();
