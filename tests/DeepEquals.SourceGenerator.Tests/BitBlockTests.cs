...
public sealed class BitBlockTests
{
    ...
    public void Bit_block_leaves_are_classified(string type, string shortName, bool bitBlock)
    {
        ...
        if (bitBlock)
            core.Should().Contain("_DeqBlocks.BlockEquals<", $"{type} is its bytes");
        else
    ...
    public void Structs_without_padding_are_bit_blocks_and_padded_ones_are_not(string declaration, bool bitBlock)
    {
        ...
        if (bitBlock)
        {
            core.Should().Contain("if (S_IsBitBlock)").And.Contain("_DeqBlocks.BlockEquals<global::Tests.S>(xs, ys)");
            run.GeneratedSource.Should().Contain("private static readonly bool S_IsBitBlock = _DeqBlocks.HasSize<global::Tests.S>(");
    ...
    public void Nested_bit_block_structs_check_every_size()
    {
        ...
        run.GeneratedSource.Should().Contain("HasSize<global::Tests.S>(16) && _DeqBlocks.HasSize<global::Tests.Inner>(8)");
    }
    ...
    public void Bit_block_spans_compare_as_bytes_and_hash_through_XxHash3()
    {
        ...
        source.Should().Contain($"_DeqBlocks.HashBlock<double>(o)", "an array hashes its own span");
        source.Should().Contain($"_DeqBlocks.HashBlock<global::System.Guid>(global::System.Runtime.InteropServices.CollectionsMarshal.AsSpan(o))");
        source.Should().Contain($"_DeqBlocks.HashBlock<decimal>(o.AsSpan())");
        source.Should().Contain($"_DeqBlocks.HashReadOnlyList<float>(");
        source.Should().Contain($"_DeqBlocks.HashList<long>(");
        source.Should().Contain($"_DeqBlocks.HashEnumerable<global::Tests.Point3>(");
        source.Should().Contain($"_DeqBlocks.HashBlock<int>(o.Span)");
        Regex.IsMatch(source, @"_DeqBlocks\.\w+<(string|bool|int\?)>").Should().BeFalse("strings, bools and nullables are not bit blocks");
    }
