...
public sealed class BasicGenerationTests
{
    ...
    public void Output_is_one_context_file_plus_one_file_per_emitted_type()
    {
        ...
        // The context file alone carries the notice's text and the description; a type file has the one-line marker.
        files["Tests.Ctx.g.cs"].Should().Contain("GetEqualityComparer<T>()").And.Contain("Options in effect").And.NotContain("Registered roots", "the files generated show the roots")
            .And.Contain("// <auto-generated>\n//     This code was generated by the DeepEquals source generator")
            .And.NotContain("Closure", "the header does not list the discovered types");
        files["Tests.Ctx.Person.g.cs"].Replace("\r\n", "\n").Should().StartWith("// <auto-generated/>\n\n#nullable enable\n\nnamespace Tests;\n")
            .And.Contain("class PersonEqualityComparer")
            .And.NotContain("This code was generated").And.NotContain("/*").And.NotContain("Options in effect");
    ...
    public void Trimming_notices_sit_in_the_files_of_the_types_they_affect()
    {
        ...
        files["Tests.Ctx.Holder.g.cs"].IndexOf("// Trimming:", StringComparison.Ordinal).Should()
            .BeGreaterThan(files["Tests.Ctx.Holder.g.cs"].IndexOf("// <auto-generated/>", StringComparison.Ordinal), "the notice follows the auto-generated marker");
        files["Tests.Ctx.Holder.g.cs"].Should().StartWith("// <auto-generated/>");
    }
    ...
    public void Files_name_the_framework_through_aliases_inside_the_namespace(LanguageVersion languageVersion)
    {
        var run = GeneratorHost.Run(Prelude.Replace("namespace Tests;", "namespace Tests {") +
                                    """
                                    public sealed class Person { public string Name = ""; public double Score; }
                                    public sealed class Plain { public int X; }
        ...
        var files = run.Result.Results.Single().GeneratedSources.ToDictionary(s => s.HintName, s => s.SourceText.ToString().Replace("\r\n", "\n"));
        var person = files["Tests.Ctx.Person.g.cs"];
        var aliases = "using _DeqHashCode = global::DeepEquals.SourceGeneration.Framework.DeepEqualsHashCode;\n" +
                      "using _DeqHelpers = global::DeepEquals.SourceGeneration.Framework.DeepEqualsHelpers;\n";
        if (languageVersion == LanguageVersion.Latest)
            person.Should().Contain("\nnamespace Tests;\n\n" + aliases + "\npublic partial class Ctx\n{\n");
        else
            person.Should().Contain("\nnamespace Tests\n{\n    " + aliases.Replace("\nusing", "\n    using") + "\n    public partial class Ctx\n    {\n");

        person.Should().Contain("_DeqHelpers.DoubleBits(x.Score)").And.Contain("_DeqHashCode.Combine(");
        files.Values.Should().OnlyContain(f => !f.Contains("global::DeepEquals.SourceGeneration.Framework.DeepEqualsHelpers.") &&
                                               !f.Contains("global::DeepEquals.SourceGeneration.Framework.DeepEqualsHashCode."),
            "every use goes through the alias");
        files["Tests.Ctx.Plain.g.cs"].Should().NotContain("_DeqHelpers", "a file declares only the aliases it uses");

        var comparer = run.Comparer("Ctx", "Person");
        var a = run.New("Person"); var b = run.New("Person");
        Set(a, "Score", 1.5); Set(b, "Score", 1.5);
    ...
    /// <summary>
    /// The aliases sit inside the namespace declaration, so a type of the same name in an enclosing namespace and a
    /// global using alias of the same name are both shadowed rather than clashing.
    /// </summary>
    [Fact]
    public void Aliases_shadow_same_named_outer_types_and_global_usings()
    {
        var run = GeneratorHost.Run("""
                                    global using _DeqHelpers = System.String;
                                    using DeepEquals.SourceGeneration;
                                    namespace Outer { public sealed class _DeqHashCode { } }
                                    namespace Outer.Tests
                                    {
                                        public sealed class Person { public double Score; public string? Name; }
