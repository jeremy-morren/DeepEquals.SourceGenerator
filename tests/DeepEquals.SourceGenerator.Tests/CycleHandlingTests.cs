...
public sealed class CycleHandlingTests
{
    ...
    internal static GeneratorRun Clean(string source) => GeneratorHost.RunClean(Prelude + source);
    ...
    public void Tree_emits_depth_parameters_and_no_state()
    {
        var run = TreeRun();
        var source = run.GeneratedSource;
        source.Should().NotContain("_DeqState").And.NotContain("TryEnter").And.NotContain("try\n");
    ...
    public void Tree_boxed_struct_cycle_through_object_throws()
    {
        ...
        run.GeneratedSource.Should().Contain("_DeqHelpers.Descend(depth, MaxDepth, typeof(global::Tests.Cell))", "the hash guards the boxed struct in its argument");
