...
| 17 rely on the BCL, not framework wrappers | Done | By request: the framework ships as little as possible. Generated code calls `Nullable.GetValueRefOrDefaultRef` directly, probed on the core library, and the header suppresses the CS9192 and CS9193 its `ref readonly` parameter raises; `DeepEqualsHelpers.NullableValueRef` from step 16 is gone. Two more wrappers went the same way: `AsWritableRef` (generated code calls `Unsafe.AsRef(in x)`) and `HalfBits` (it calls `BitConverter.HalfToUInt16Bits`), and the unused `GetNullableValueRefOrDefaultRef`. The remaining helpers do work the BCL has no call for: per-asset float and double reads, decimal, Guid and DateTime storage, the constrained `EquatableEquals`, and the throw helpers. |
| 18 header suppressions and obsolete types | Done | By request. The header writes one `#pragma warning disable CSnnnn // Suppress …` line per warning the generator's own code raises, then after a blank line one per warning the compared types' APIs raise (CS0612, CS0618, custom `[Obsolete]` ids, `[Experimental]` and preview ids, each naming its symbol; custom comparer types and property getters are now visited too). A member obsolete as an error was read through its getter and broke the build with CS0619; it is now read through its storage. The comparer class and convenience property of an obsolete type, including one built from an obsolete type argument, repeat its `[Obsolete]` (message, error flag, `DiagnosticId`, `UrlFormat`). A type obsolete as an error compiles when the context is `[Obsolete]`, and otherwise reports the new `DEQ038`. Tests: `ObsoleteTests`, six cases covering the header layout, warning- and error-level members, obsolete types and type arguments, custom ids, an obsolete context and `DEQ038`. |
